...
func keepRunning(ctx context.Context, tag, address string, joinAddrs []string, hostname string) {
	...
	for {
		...
		if pid != 0 && isSerfAgent(pid) {
			...
			args := []string{
				...
				fmt.Sprintf("-rpc-addr=%s -tag network=%s -node=%s", address, tag, hostname),
			}
